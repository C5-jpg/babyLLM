...
用法:
  python evaluate_blimp_v4.py --model_dir output/babylm-llama-v4/best_model --use_demo_data
  python evaluate_blimp_v4.py --model_dir output/babylm-llama-v4/best_model --data_file blimp_data.json
"""
import os
import sys
import math
...
def sentence_log_prob(model, tokenizer, sentence: str, device: str) -> float:
    """
    计算句子的平均 token log-probability（越高越好）。
    使用 teacher-forcing: 模型看到完整序列，预测每个 token，取平均 log-prob。
    """
    input_ids = tokenizer.encode(sentence, add_special_tokens=False, return_tensors="pt").to(device)
    if input_ids.shape[1] == 0:
        return -float('inf')

    with torch.no_grad():
        outputs = model(input_ids=input_ids, labels=input_ids)
        # outputs.loss 是平均负 log-likelihood（对所有 token）
...
def evaluate_blimp(
    ...
    pairs: list,
    device: str = "cuda",
    batch_info: bool = True,
) -> dict:
    """
    对 BLiMP 句对列表进行评测。

    每个 pair 包含:
      - sentence_good: 语法正确的句子
      - sentence_bad:  语法错误的句子
      - category (可选): 语法类别标签

    返回:
      dict，包含 accuracy、category_accuracy 等统计信息
    """
    model.eval()
    results = []
    correct = 0

"""
ChineseBabyLM V14 — Optimized Efficiency Build (based on deep analysis + BabyLM research)

Based on V1-V13 analysis + BabyLM 2024/2025 winning techniques:
  - Problem: V13 (94.2M params) tokens/param=1.1x, far below ideal 20-200x
  - Solution: Shrink model to ~52M params, tokens/param=1.9x
  - V13 best PPL=38.68 with 94.2M params → V14 targets PPL~37.5 with ~52M params
  - V13 Stage 3 DropBlock+StochDepth caused negative optimization → removed

V14 changes vs V13:
  - Architecture: 640d, 12L, 10Q/5KV GQA (~52M params, was 94.2M)
  - Better tokens/param ratio: 1.9x (was 1.1x)
  - 5-stage pipeline: CLM→MNTP→Polish→Self-Distill→Annealing
  - Stage 3: Pure polish, NO DropBlock/StochDepth (V13 lesson learned)
  - Larger effective batch: 24x4x2=192 (was 16x4x2=128)
  - Enhanced Focal Loss: gamma=2.0 (was 1.5)
  - MNTP mask_ratio_end=0.15 (conservative, per GPT-BERT findings)
  - Self-distillation added as Stage 4
  - Annealing + data replay as Stage 5

Architecture: LlamaForCausalLM, 640d, 12L, 10Q/5KV GQA, SPM 32K, ~52M params
"""
...
def build_model(args, tokenizer, accelerator=None):
    ...
    if args.stochastic_depth > 0:
        print(f"  Stochastic Depth: p={args.stochastic_depth} (applied via layer skip in training)")

    if args.dropblock > 0:
        print(f"  DropBlock attention: p={args.dropblock}, block_size={args.dropblock_size}")
...
def train():
    parser = argparse.ArgumentParser(description="ChineseBabyLM V14 Training")
    ...
    parser.add_argument("--sd_update_freq", type=int, default=0, help="Update teacher every N steps (0=per epoch)")
    parser.add_argument("--stochastic_depth", type=float, default=0.0, help="Layer drop probability")
    parser.add_argument("--dropblock", type=float, default=0.0, help="DropBlock probability for attention")
    parser.add_argument("--dropblock_size", type=int, default=3, help="DropBlock block size")
    parser.add_argument("--resume_from_checkpoint", default="", help="Path to checkpoint dir to resume from (restores optimizer/scheduler/step/epoch)")
    parser.add_argument("--eval_steps", type=int, default=500, help="Run validation every N steps (0=epoch-only)")
    parser.add_argument("--early_stop_min_delta", type=float, default=1e-4, help="Min val_loss improvement to reset patience")
    parser.add_argument("--early_stop_patience", type=int, default=3, help="Step-level early stopping patience")
    ...
    if accelerator.is_main_process:
        print("=" * 60)
        print(f"  ChineseBabyLM V14 — Stage: {args.stage.upper()}")
        print("=" * 60)
    ...
    if args.self_distill and ema is not None:
        ...
        if accelerator.is_main_process:
            print(f"  Self-Distillation: T={args.sd_temperature}, lambda={args.sd_lambda}, update_freq={args.sd_update_freq}")
    ...
    if accelerator.is_main_process:
        ...
        run_name = args.wandb_run_name or f"babylm-v14-{args.stage}"
        resume_id = None
        if resume_ckpt:
        ...
        wandb_run.alert(
            title=f"V14 {args.stage.upper()} {'Resumed' if resume_ckpt else 'Started'}",
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in tqdm(
            ...
            if teacher_model is not None and args.sd_update_freq > 0 and global_step % args.sd_update_freq == 0:
                sd_state = {k: v.to(v.dtype) for k, v in ema.shadow.items()}
                teacher_model.load_state_dict(sd_state, strict=False)
                teacher_model.eval()
            ...
            # Step-level validation checkpoint
        ...
        if accelerator.is_main_process:
            ...
            if val_loss_t < best_val:
                ...
                wandb_run.alert(
                    title=f"V14 {args.stage.upper()} New Best PPL={ppl:.2f}",
                    text=(
        ...
        if args.patience > 0 and patience_tensor >= args.patience:
            if accelerator.is_main_process:
                ...
                wandb_run.alert(
                    title=f"V14 {args.stage.upper()} Early Stopped",
    ...
    if accelerator.is_main_process:
        ...
        wandb_run.alert(
            title=f"V14 {args.stage.upper()} Complete — PPL={best_ppl:.2f}",
            text=summary,
            level=wandb.AlertLevel.INFO,
        )

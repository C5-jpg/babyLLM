"""
ChineseBabyLM V12 — Optimized Training Script

V12 results: PPL=40.72, 38.7M params, ZhoBLiMP=61.97, AFQMC=69.07%
V12 changes:
  - Larger model: 576d, 14L, 9Q/3KV GQA, ~54M params
  - Focal Loss for Stage 1-2 (gamma=2.0, hard example mining)
  - Improved Self-Distillation: T=4.0, sd_lambda=0.7, step-level teacher update
  - Better default hyperparams: patience=8, longer training
  - Data cleaning: dedup + quality filter
  - 5-stage pipeline (no SWA)

Architecture: LlamaForCausalLM, 576d, 14L, 9Q/3KV GQA, 8K Unigram, ~54M params
"""
...
def train():
    parser = argparse.ArgumentParser(description="ChineseBabyLM V12 Training")
    ...
    if accelerator.is_main_process:
        print("=" * 60)
        print(f"  ChineseBabyLM V12 — Stage: {args.stage.upper()}")
        print("=" * 60)
    ...
    if accelerator.is_main_process:
        ...
        run_name = args.wandb_run_name or f"babylm-v12-{args.stage}"
        wandb_run = wandb.init(project=args.wandb_project, name=run_name, config=vars(args))

        wandb_run.alert(
            title=f"V12 {args.stage.upper()} Started",
    ...
    for epoch in range(args.epochs):
        ...
        if accelerator.is_main_process:
            ...
            if val_loss_t < best_val:
                ...
                wandb_run.alert(
                    title=f"V12 {args.stage.upper()} New Best PPL={ppl:.2f}",
                    text=(
        ...
        if args.patience > 0 and patience_tensor >= args.patience:
            if accelerator.is_main_process:
                ...
                wandb_run.alert(
                    title=f"V12 {args.stage.upper()} Early Stopped",
    ...
    if accelerator.is_main_process:
        ...
        wandb_run.alert(
            title=f"V12 {args.stage.upper()} Complete — PPL={best_ppl:.2f}",
            text=summary,
            level=wandb.AlertLevel.INFO,
        )

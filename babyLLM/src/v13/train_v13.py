"""
ChineseBabyLM V13 — SOTA Competition Build

V13 results: PPL=38.84, ZhoBLiMP=62.03, AFQMC=69.0%
V13 changes:
  - Maximal model: 768d, 14L, 12Q/4KV GQA, ~94.2M params
  - Data: PPL filter + MinHash dedup + quality filter + hard upsample
  - 3-stage pipeline only (CLM→MNTP→Polish, no SD/Annealing)
  - DropBlock attention + Stochastic Depth in Stage 3
  - Curriculum Focal Loss (γ: 1.5→1.0→0)
  - Flash Attention 2

Architecture: LlamaForCausalLM, 768d, 14L, 12Q/4KV GQA, 8K Unigram, ~94.2M params
"""
...
def train():
    parser = argparse.ArgumentParser(description="ChineseBabyLM V13 Training")
    ...
    if accelerator.is_main_process:
        print("=" * 60)
        print(f"  ChineseBabyLM V13 — Stage: {args.stage.upper()}")
        print("=" * 60)
    ...
    if accelerator.is_main_process:
        ...
        run_name = args.wandb_run_name or f"babylm-v13-{args.stage}"
        resume_id = None
        if resume_ckpt:
        ...
        wandb_run.alert(
            title=f"V13 {args.stage.upper()} {'Resumed' if resume_ckpt else 'Started'}",
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if accelerator.is_main_process:
            ...
            if val_loss_t < best_val:
                ...
                wandb_run.alert(
                    title=f"V13 {args.stage.upper()} New Best PPL={ppl:.2f}",
                    text=(
        ...
        if args.patience > 0 and patience_tensor >= args.patience:
            if accelerator.is_main_process:
                ...
                wandb_run.alert(
                    title=f"V13 {args.stage.upper()} Early Stopped",
    ...
    if accelerator.is_main_process:
        ...
        wandb_run.alert(
            title=f"V13 {args.stage.upper()} Complete — PPL={best_ppl:.2f}",
            text=summary,
            level=wandb.AlertLevel.INFO,
        )

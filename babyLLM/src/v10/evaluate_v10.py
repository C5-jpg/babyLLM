"""
ChineseBabyLM V10 — Evaluation Script
...
def compute_ppl(model, sp, val_file, device, max_length=1024):
    ...
    with torch.no_grad():
        for i in tqdm(
            range(0, len(all_tokens) - max_length, max_length), desc="Evaluating"
        ):
...
def generate_samples(model, sp, device, prompts, max_new_tokens=64):
    ...
    for prompt in prompts:
        ...
        results.append(
            {"prompt": prompt, "greedy": greedy_text, "sampled": sample_text}
        )
    return results


def main():
    parser = argparse.ArgumentParser(description="ChineseBabyLM V10 Evaluation")
    parser.add_argument("--model_path", required=True)
    parser.add_argument("--val_file", required=True)
    parser.add_argument("--max_length", type=int, default=1024)
    parser.add_argument("--output_json", default="", help="Save results as JSON")
    args = parser.parse_args()

    device = "cuda" if torch.cuda.is_available() else "cpu"

    print("=" * 60)
    print("ChineseBabyLM V10 - Evaluation")
    print("=" * 60)

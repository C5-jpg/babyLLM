...
mkdir -p "$HDD_OUT"
cd "$PROJECT_DIR"
echo "=== V9 Probe: MNTP rebalance from v8 best ==="
S1_OUT="$HDD_OUT/probe_mntp_stride512"
CUDA_VISIBLE_DEVICES=0,1,2,3 accelerate launch \
    ...
    --lr 1.5e-4 --epochs 1 --batch_size 16 --stride 512 \
    --mask_ratio_start 0.25 --mask_ratio_end 0.12 --clm_ratio 0.125 \
    --label_smoothing 0.02 --max_steps 800
echo "=== V9 Probe Evaluation ==="
python src/v9/evaluate_v9.py --model_path "$S1_OUT/best_model" --val_file "$DATA_DIR/val.txt" > "$HDD_OUT/eval_probe.json"
cat "$HDD_OUT/eval_probe.json"
